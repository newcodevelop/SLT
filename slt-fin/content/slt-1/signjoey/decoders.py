...
class TransformerDecoder(Decoder):
    ...
    def __init__(
        ...
        **kwargs
        ...
        :param vocab_size: size of the output vocabulary trans
        :param vocab_size_recog: size of the output vocabulary recog
        ...
        self.output_layer = nn.Linear(hidden_size, vocab_size, bias=False)
        self.output_layer_recog = nn.Linear(hidden_size, vocab_size_recog, bias=False)
        self.output_layer_en = nn.Linear(hidden_size,3918, bias=False)
        self.layer_specific_trans = TransformerDecoderLayer(
                    size=hidden_size,
                    ff_size=ff_size,
                    num_heads=num_heads,
                    dropout=dropout,
                )
        self.layer_specific_recog = TransformerDecoderLayer(
                    size=hidden_size,
                    ff_size=ff_size,
                    num_heads=num_heads,
                    dropout=dropout,
                )
        self.layer_specific_en = TransformerDecoderLayer(
                    size=hidden_size,
                    ff_size=ff_size,
                    num_heads=num_heads,
                    dropout=dropout,
    ...
    def forward(
        ...
        **kwargs
        ...
        x = self.layer_specific_trans(x=x, memory=encoder_output, src_mask=src_mask, trg_mask=trg_mask)
        x = self.layer_norm(x)
        output = self.output_layer(x)
        y = self.layer_specific_recog(x=x, memory=encoder_output, src_mask=src_mask, trg_mask=trg_mask)
        y = self.layer_norm(y)
        y = self.output_layer_recog(y)

        z = self.layer_specific_en(x=x, memory=encoder_output, src_mask=src_mask, trg_mask=trg_mask)
        z = self.layer_norm(z)
        z = self.output_layer_en(z)
        return output, x, None, None,y,z
