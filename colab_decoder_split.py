...
my_parser.add_argument('-BUFFER_SIZE', action='store', type=int, default=1000)
...
my_parser.add_argument('-use_colab', action='store', type=int, required=True)
my_parser.add_argument('-dropout_rate', action='store', type=float, required=True)
...
if args.use_colab:
  ...
  """
  os.makedirs('train')
  ref = np.random.randn(475,1024)
  for i,val in tqdm(zip(names,videos)):
    sequence = [val,ref]
  ...
  logging.info('train caption')
  logging.info(train_captions_en)
  ...
  logging.info('captions shape {}'.format(np.shape(train_captions_final)))
  logging.info('glosses shape {}'.format(np.shape(train_glosses_final)))
  ...
  def create_padding_mask_image(videos):
    ...
    #return mask[:, tf.newaxis, tf.newaxis, :]
    return np.asarray(mask)
  video_mask = create_padding_mask_image(videos)
  ...
  dataset = tf.data.Dataset.from_tensor_slices((names, train_captions_final,train_glosses_final,video_mask,train_captions_en))
  ...
  logging.info('dataset loaded from .npy')
  # create tfrecords without shards for faster access
  """
  for i, _ in enumerate(dataset.element_spec):
    ds_i = dataset.map(lambda *args: args[i]).map(tf.io.serialize_tensor)
    writer = tf.data.experimental.TFRecordWriter(f'mydata.{i}.tfrecord')
    writer.write(ds_i)
  #delete train dir containing videos to save disk space
      
  shutil.rmtree('train')
  logging.info('created tfrecords')
  # recreate dataset from tfrecords for faster access rather than loading from .npy files in parallel
  parts = []
  def read_map_fn1(x):
  ...
  logging.info('dataset recreated from tfrecords')
  # Shuffle and batch


  dataset = dataset.shuffle(BUFFER_SIZE).padded_batch(BATCH_SIZE,([None,None],[None],[None],[None],[None]))

  logging.info('faster dataset created')
...
checkpoint_path = "./checkpoints/train/pman/threechannels-80epoch"
...
def translate(img,plot=''):
  ...
  #predicted_sentence_en = tokenizer_en.decode([i for i in result_en])
  ...
  #print('Predicted translation eng: {}'.format(str(predicted_sentence_en)))
...
for i in range(len(p1)):
  ...
  names_test.append(p1[i]['name'])
names_test = list(map(lambda x: 'span=12_stride=2/'+x.split('/')[1]+'.pt',names_test))
...
if __name__== "__main__":
  ...
  ref,hyp1,temp = [],[],[]
  for m,n in tqdm(zip(names_test,captions_test)):
    #translated1,translated2,translated3 = translate(m,False)
    m = np.asarray(torch.squeeze(torch.stack(torch.load(m)),dim=1),dtype = np.float32)
